...
docker run -itd --name D1 --net bridge1 --ip 10.0.10.254 alpine sh # kontener D1 na bazie alpine dolaczony do sieci bridge1 z adresem ip 10.0.10.250
...
docker run -itd --name S1 --net bridge2 ubuntu sh # kontener S1 na bazie ubuntu w sieci bridge2

docker run -itd --name D2 --net bridge1 -p 10.0.10.0:8080:80 -p 10.0.2.15:8081:80 busybox sh # kontener D2 na bazie apache w sieci bridge1 z mapowaniem na port 8080 a dla sieci 10.0.2.15 port mapowany na 8081

docker network connect bridge2 D2 # polaczenie kontenera D2 do sieci bridge2